...
from conversation_qa_scorecard.adapters._review_payload import (
    scorecard_to_review,
)
...
from .canonical import AS_OF, CANONICAL_CALLS, CANONICAL_CONTACT, CANONICAL_SCORECARD
...
def test_the_offline_outbox_flushes_the_payload_the_managed_router_would_submit() -> None:
    ...
    router.route(CANONICAL_SCORECARD, maker=sample_cases.ACTOR, tenant=sample_cases.TENANT)
    ...
    expected = scorecard_to_review(
        CANONICAL_SCORECARD, maker=sample_cases.ACTOR, tenant=sample_cases.TENANT
    ).to_payload()
...
def test_the_payload_that_reaches_the_wire_is_redacted_whichever_family_built_it() -> None:
    ...
    contact = next(
        c for c in container.transcription.contacts() if c.contact_id == sample_cases.PII_CONTACT
    )
    ...
    payload = repr(
        scorecard_to_review(
            scorecard, maker=sample_cases.ACTOR, tenant=sample_cases.TENANT
        ).to_payload()
    )
...
def test_the_whole_pipeline_answers_on_local_and_fails_fast_on_onprem() -> None:
    ...
    contact = next(
        c for c in local.transcription.contacts() if c.contact_id == sample_cases.BREACH_CONTACT
    )
    scorecard = build_service(local).score_contact(
        contact,
        pack_for_contact(local, contact),
        actor=sample_cases.ACTOR,
        tenant=sample_cases.TENANT,
        as_of=AS_OF,
    )
    assert scorecard.requires_human_review is True
    assert scorecard.citations, "an offline answer must still be cited"
    assert scorecard.review_ref, "rule R8: the domain service routes inside the same call"

    onprem = build_container(local_settings(profile="onprem"))
    with pytest.raises(NotImplementedError):
        build_service(onprem).score_contact(
            CANONICAL_CONTACT,
            pack_for_contact(onprem, CANONICAL_CONTACT),
            actor=sample_cases.ACTOR,
            tenant=sample_cases.TENANT,
            as_of=AS_OF,
        )
...
def test_the_tenant_boundary_holds_before_any_transcript_is_fetched() -> None:
    """A caller must not be able to make this service transcribe another tenant's recording."""
    local = build_container(local_settings())
    other = next(
        c
        for c in local.transcription.contacts()
        if c.contact_id == sample_cases.OTHER_TENANT_CONTACT
    )
    with pytest.raises(TenantAccessDeniedError):
        build_service(local).score_contact(
            other,
            pack_for_contact(local, other),
            actor=sample_cases.ACTOR,
            tenant=sample_cases.TENANT,
            as_of=AS_OF,
        )
