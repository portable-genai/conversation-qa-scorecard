...
def test_the_demo_data_is_obviously_fictional() -> None:
    """Every conversation the demo scores is synthetic, and says so in the file it lives in."""
    ...
    source = FixtureTranscriptSource(Settings(profile="local"))
    contact_ids = (demo.COMPLIANT_CONTACT, demo.BREACH_CONTACT, demo.PII_CONTACT)
    blob = demo.ACTOR
    for contact_id in contact_ids:
        transcript = source.fetch(contact_id)
        blob += " " + " ".join(turn.text for turn in transcript.turns)
    assert "FICTIONAL" in blob, "the demo parties do not announce themselves as fake"
...
def test_every_shipped_transcript_declares_itself_synthetic() -> None:
    """A fixture that reads like a real call must carry the notice that it is not one."""
    import json
    ...
    files = sorted(BUNDLED_FIXTURES.glob("*.json"))
    assert files, "the demo has no shipped transcripts to score"
    for path in files:
        document = json.loads(path.read_text(encoding="utf-8"))
        assert "SYNTHETIC" in str(document.get("notes", "")), f"{path.name} claims no provenance"
