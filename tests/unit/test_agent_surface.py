...
from conversation_qa_scorecard.agent import (
    SKILLS,
    TOOL_FUNCTIONS,
    agent_card_document,
    build_agent_card,
...
def test_the_card_document_is_json_safe_and_names_the_region() -> None:
    ...
    assert document["name"] == "conversation-qa-scorecard"
...
def test_the_agent_path_routes_a_failing_scorecard_rather_than_only_flagging_it() -> None:
...
def test_the_agent_path_does_not_manufacture_a_review_for_a_passing_contact() -> None:
    result = score_contact(
...
def test_the_agent_surface_refuses_another_tenants_contact() -> None:
    """The tenant boundary is in the DOMAIN, so it holds on this surface without being re-added."""
    from conversation_qa_scorecard.domain.errors import TenantAccessDeniedError

    with pytest.raises(TenantAccessDeniedError):
        score_contact(
            sample_cases.OTHER_TENANT_CONTACT,
            tenant=sample_cases.TENANT,
            settings=local_settings(),
        )
...
def test_the_tool_output_is_masked_before_it_can_reach_a_model() -> None:
    ...
    The transcript was already masked at ingestion, so this is the second pass: it walks the
    whole structure rather than three named fields, so a field added later cannot arrive
    ...
    assert result["redaction_count"] >= 1, (
        "the scorecard must be able to show a reviewer that redaction ran at all; a zero here "
        "would make the assertion above vacuous on a contact that carried no identifier"
    )
