...
_ANCHOR_ENV = "CONVQA_AUDIT_ANCHOR"
...
def _event(contact_id: str, disposition: str) -> AuditEvent:
    """The shape the scorecard service writes: already redacted, cited, severity-banded."""
    return AuditEvent(
        action="score_contact",
        actor=sample_cases.ACTOR,
        decision=Decision.ESCALATED if disposition != "compliant" else Decision.ALLOWED,
        severity=Severity.CRITICAL if disposition != "compliant" else Severity.LOW,
        redacted_summary=f"{contact_id}: {disposition} on SG/{sample_cases.SG_PACK_ID}",
        citations=(Citation(source_id="sg_mas_fair_dealing", title="Fair dealing"),),
    )


def _record_three(settings: Settings) -> LocalAuditAdapter:
    """Drive the REAL bound adapter, so what gets anchored is what the service writes."""
    container = build_container(settings)
    audit = container.audit
    assert isinstance(audit, LocalAuditAdapter)
    for contact_id, disposition in (
        (sample_cases.BREACH_CONTACT, "non_compliant"),
        (sample_cases.COMPLIANT_CONTACT, "compliant"),
        (sample_cases.PII_CONTACT, "remediate"),
    ):
        audit.record(_event(contact_id, disposition))
    return audit
...
def test_an_append_after_truncation_cannot_relaunder_the_anchor(tmp_path: Path) -> None:
    ...
    with pytest.raises(AuditChainError):
        audit.record(_event(sample_cases.BREACH_CONTACT, "non_compliant"))
