...
def test_an_env_var_set_to_empty_does_not_inherit_the_files_default(
    ...
    """Three-state expansion: unset takes the written default, a value wins."""
...
def test_an_env_var_set_to_empty_refuses_rather_than_resolving_to_empty(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """The emptied half, which used to resolve to the empty string and now refuses.

    ``${VAR:-default}`` is ``setting_or_default(VAR, default)`` one layer down, so it obeys the
    same rule. Resolving to empty made ``${VAR:-http://audit:8080}`` with ``VAR=""``
    ...
    written, so the refusal cannot be delegated downstream; it lands at boot, in the deploy, with
    the variable and its documented default named.
    """
    path = tmp_path / "settings.yaml"
    path.write_text("region: ${TEMPLATE_TEST_REGION:-fallback-region}\n", encoding="utf-8")
