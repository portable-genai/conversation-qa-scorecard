...
from conversation_qa_scorecard.adapters.gcp.identity import (
    _AUDIENCE_ENV,
...
from conversation_qa_scorecard.ports.identity import (
    VERIFIED,
...
GOOD_CLAIMS: dict[str, Any] = {
    ...
    # The hosted-domain claim IS the tenant partition in this deployment, so it names the
    # partition the synthetic contact catalogue uses. Mapping an IdP claim to a partition is
    # deployment configuration; what matters here is that the DOMAIN compares against it.
    "hd": "demo-bank",
...
_API_MODULE = "conversation_qa_scorecard.api.app"
...
def _offline_binding(port: str, offline: str, exit_target: str) -> list[str]:
    """Bind one port to the SDK-free adapter under BOTH local and gcp, and the exit placeholder.

    Only the IDENTITY port is left on its managed adapter, because it is the one under test.
    Everything else runs offline so these tests need no Cloud Logging, no Firestore, no
    recogniser and no live console. This is the documented rebinding path (``adapters:`` in the
    settings file), not a test-only hook.
    """
    return [
        f"  {port}:",
        *[f"    {profile}: {_PKG}.adapters.local.{offline}" for profile in ("local", "gcp")],
        f"    onprem: {_PKG}.adapters.onprem.{exit_target}",
    ]
...
_REBOUND_SETTINGS = "\n".join(
    [
        ...
        *_offline_binding("narration", "narration:LocalNarrator", "narration:OnPremNarrator"),
        # The observability ports take no part in the identity posture under test, but every
        # port must bind every profile or `_bindings_from` refuses the whole file.
        *_offline_binding("tracer", "tracer:LocalNoopTracerAdapter", "tracer:OnPremTracerAdapter"),
        *_offline_binding(
            "evaluation",
            "evaluation:LocalOfflineEvalAdapter",
            "evaluation:OnPremEvalAdapter",
        ),
        *_offline_binding(
            "review_router",
            "review_router:LocalReviewRouter",
            "review_router:OnPremReviewRouter",
        ),
        *_offline_binding(
            "scorecard_store",
            "scorecard_store:LocalScorecardStore",
            "scorecard_store:OnPremScorecardStore",
        ),
        *_offline_binding(
            "signal_classifier",
            "signals:LocalSignalClassifier",
            "signals:OnPremSignalClassifier",
        ),
        *_offline_binding(
            "transcription",
            "transcription:FixtureTranscriptSource",
            "transcription:OnPremTranscriptSource",
        ),
        *_offline_binding(
            "warehouse", "warehouse:LocalWarehouseExport", "warehouse:OnPremWarehouseExport"
        ),
    ]
)
...
def _managed_app(monkeypatch: pytest.MonkeyPatch, tmp_path: Any, audience: str) -> Any:
    ...
    supplies it (three-state: unset takes the written default, an emptied variable REFUSES at
    load rather than resolving to empty).

    ``audience=""`` therefore means UNCONFIGURED, and it is expressed by unsetting the variable,
    which is how a deployment that never configured it actually looks. Blanking it is a
    different, louder case: the loader refuses and the process never starts, which
    ``test_an_emptied_audience_refuses_at_load_rather_than_resolving_to_empty`` pins.
    """
