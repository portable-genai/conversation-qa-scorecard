...
Conversation QA and Compliance Scorecard (E3): Post-contact deterministic conversation QA and compliance scorecard with cited disposition.
Rendered from `hex-service-template`, so it starts at reference parity rather than converging
toward it. Package `conversation_qa_scorecard`, environment prefix
`CONVQA`, region `asia-southeast1`.
...
After any dependency change, run `make lock` and commit `uv.lock` with both exported lockfiles. An uncommitted resolution
...
- `src/conversation_qa_scorecard/domain/` is PURE stdlib plus the pinned speech kernel. No web
  framework, no cloud SDK, no HTTP, no clock. `kernel.py` holds the vertical-neutral machinery;
  everything else is this vertical: `models.py` (requirements, findings, evidence spans,
  signals, the scorecard), `ingestion.py` (turn assembly and redaction, before any model),
  `scoring_engine.py` (the engine that owns every number and every verdict),
  `narration.py` (the grounding gate a model draft must pass), `scorecard_service.py` (the one
  path a contact takes) and `serialization.py` (the record, both ways).
- `score_pack.py` and `scorepacks/` hold the policy: what each market mandates, as YAML a
  compliance officer can diff. It lives OUTSIDE `domain/` because it reads a file.
- `service.py` is the single wiring seam between `config.Container` and the domain service, so
  no surface can build a narrower one and quietly stop honouring rule R8.
- `ports/` holds `@runtime_checkable` Protocols, re-exported once from `ports/__init__.py` with
  ...
  `ports/speech.py` RE-EXPORTS the pinned `speech-lexicon-kit` transcript types and speech ports
  rather than redeclaring them; a local copy is how three sibling systems stop agreeing on what
  "turn 4, characters 12 to 34" means. Only `TranscriptSourcePort` is this repo's own.
...
- **The exposure guard is derived from the IDENTITY BINDING, and from nothing else.** An end-user
  ...
  `CONVQA_S2S_TOKEN` may never enter that decision: it authenticates a
  ...
  persona and a real scorecard. `tests/unit/test_end_user_auth_posture.py` walks the
...
- **The one adapter that declares `VERIFIED` must EARN it, and it is the one adapter that may not
  ...
  configured `CONVQA_IAP_AUDIENCE`, three-state: unset or emptied REFUSES,
...
- **One profile read, three states, refused at import.** `config.PROFILE_CHOICE` resolves once at
  ...
  `CONVQA_PROFILE` (mentioning it in a refusal message is wanted, reading
...
- **Rule R8: escalations are ROUTED, never merely flagged.** Setting `requires_human_review` and
  ...
  and green.
- **The consequential decision is deterministic.** Every requirement status, both score ratios,
  the sentiment sum, the severity and the disposition come from `domain/scoring_engine.py`: pure
  stdlib plus the pinned speech kernel, an explicit `as_of`, no clock and no I/O. A model may
  restate the result; it may never produce a number or a verdict, and a draft that mentions a
  figure the engine did not publish is discarded by `domain/narration.py`.
  `tests/unit/test_narration_and_model_freedom.py` scores the same contacts with and without the
  model adapters bound and requires every consequential field to be identical.
- **What a market requires is CONFIGURATION, never code.** The mandated wordings, the accepted
  paraphrases, the ordering constraints, the timing windows, the severities, the citations and
  the cue lexicons live in `scorepacks/reference_packs.yaml`. The engine carries no market
  vocabulary, and a source scan in `tests/unit/test_scoring_engine.py` fails the build if one
  appears.
- **The engine fails closed, in six places**, and each has a test: unmatched is ABSENT,
  out-of-order scores as absent, a declared window with no word timings is UNVERIFIABLE, an
  unconfigured declared obligation is a GAP at the top severity, no declared obligations is
  INDETERMINATE at 0.0, and the wrong speaker does not count.
- **Every result carries a citation.** A finding names its instrument AND the turn index and
  half-open character span in the redacted turn text that produced it. A claim with no
  provenance is not shippable, and a citation that points at the wrong words is worse than none.
- **The tenant boundary lives in the domain and answers 403.** The store filters on tenant for a
  listing and is deliberately unfiltered for a fetch by id, so the comparison against the
  VERIFIED principal happens once, where every surface inherits it. 404 would make the store
  probeable with an id generator. `tests/unit/test_scorecard_store_and_tenancy.py` carries a
  mutant with the check removed that MUST leak, so the denial test cannot be quietly deleted.
