"""FastAPI application for Conversation QA and Compliance Scorecard (E3).
...
``CONVQA_PROFILE`` is NO CHOICE rather than a silent ``local``. Every
...
``CONVQA_S2S_TOKEN`` is wrong in the most dangerous direction:
...
from ..contacts import (
    ContactCatalogueUnavailableError,
    ContactNotFoundError,
    contact_catalogue,
    find_contact,
    headline_for,
)
from ..domain.errors import ScorecardNotFoundError, TenantAccessDeniedError
from ..domain.kernel import utcnow
from ..ports.identity import VERIFIED, EndUserAuthUnavailableError
from ..service import build_service, pack_for_contact, redacted_transcript_for
from .schemas import ContactModel, HealthResponse, ScorecardResponse, ScoreRequest
...
_INSECURE_DEMO_ENV = "CONVQA_ALLOW_INSECURE_DEMO"
_CORS_ORIGINS_ENV = "CONVQA_CORS_ORIGINS"
_ALLOWED_CALLERS_ENV = "CONVQA_S2S_ALLOWED_CALLERS"
_AUDIENCE_ENV = "CONVQA_S2S_AUDIENCE"
_API_HOST_ENV = "CONVQA_API_HOST"
...
def require_service_caller(request: Request) -> None:
    ...
    stays OPEN when ``CONVQA_S2S_TOKEN`` is unset (loopback dev with zero
    secrets), so an UNSET ``CONVQA_PROFILE`` must never be allowed to
    ...
    if not _request_choice(request).service_auth_configured:
        raise HTTPException(
            ...
            detail=(
                ...
                "CONVQA_PROFILE is not set, so no authentication scheme "
                "has been chosen"
            ),
        )
...
#: Service name on every log line and every span. The repo slug: stable, greppable, and
#: the same string the tracer reports as `service.name`.
_SERVICE_NAME = "conversation-qa-scorecard"

# Configured at MODULE scope for the same reason the exposure guard is bound at module
# scope: the Dockerfile CMD and `make run-api` serve the app OBJECT, so anything living
# only inside main() never runs in a shipped process. A deployed profile gets JSON with
# Cloud Logging's field names and the active trace id; local and onprem stay plain text.
...
_APP_TITLE = "Conversation QA and Compliance Scorecard"
...
#
# Note what is NOT in this expression: CONVQA_S2S_TOKEN. A service
...
def _domain_error(exc: Exception) -> HTTPException:
    """Map a domain refusal to its declared HTTP status, never to a bare 500.

    The status lives on the exception CLASS in ``domain/errors.py`` and ``contacts.py``, so a
    new refusal arrives with its answer attached rather than needing a branch here. The
    important pair is 403 against 404: a scorecard owned by another tenant EXISTS and is
    refused, and answering 404 would make the store probeable with an id generator.
    """
    status_code = int(getattr(exc, "http_status", status.HTTP_400_BAD_REQUEST))
    return HTTPException(status_code=status_code, detail=str(exc))


@app.post("/v1/scorecards", response_model=ScorecardResponse, tags=["artifacts"])
def score_contact(
...
def read_scorecard(
    ...
    """One stored scorecard, authorised in the DOMAIN against the verified principal's tenant.

    Another tenant's scorecard is 403, not 404. It exists, and this caller may not have it.
    """
    try:
        scorecard = build_service(_container()).fetch(scorecard_id, tenant=principal.tenant)
    except (TenantAccessDeniedError, ScorecardNotFoundError) as exc:
        raise _domain_error(exc) from exc
    return ScorecardResponse.from_domain(scorecard)


@app.get("/v1/contacts", response_model=list[ContactModel], tags=["artifacts"])
def list_contacts(
    principal: Annotated[Principal, Depends(get_principal)],
) -> list[ContactModel]:
    """Contacts this principal's tenant may score. Filtered on the VERIFIED tenant, here.

    A profile with no contact catalogue bound answers an empty list rather than 501: a picker
    with nothing in it is the honest rendering of "this deployment resolves contacts from its
    own platform", and the 501 with the explanation is reserved for an actual scoring attempt.
    """
    container = _container()
    try:
        catalogue = contact_catalogue(container)
    except ContactCatalogueUnavailableError:
        return []
    return [
        ContactModel(
        ...
        for contact in catalogue
        if contact.tenant == principal.tenant
    ]


@app.get("/v1/contacts/{contact_id}/transcript", tags=["artifacts"])
def read_transcript(
    contact_id: str,
    principal: Annotated[Principal, Depends(get_principal)],
) -> dict[str, object]:
    """The REDACTED transcript a reviewer reads, with the same offsets the engine scored.

    The same ingestion call the scoring path makes, so a citation of "turn 4, characters 12 to
    34" resolves to the same characters here. A second, kinder rendering path is how evidence
    stops matching the finding it justifies.
    """
    container = _container()
    try:
        ...
        if contact.tenant != principal.tenant:
            raise TenantAccessDeniedError(
                f"{contact_id!r} belongs to another tenant partition; "
                "this principal may not read it"
            )
        redacted = redacted_transcript_for(container, contact)
    except (TenantAccessDeniedError, ContactNotFoundError, ContactCatalogueUnavailableError) as exc:
        raise _domain_error(exc) from exc
