...
One env var (``CONVQA_PROFILE``) selects the adapter family for every
...
from .ports.signals import SignalClassifierPort
from .ports.speech import TranscriptSourcePort
from .ports.warehouse import WarehouseExportPort

_PROFILE_ENV = "CONVQA_PROFILE"
_SETTINGS_ENV = "CONVQA_SETTINGS"
...
_GENERATOR_MODEL_ATTR: str = "narration_model"
...
class ProfileChoice:
    ...
    The variable is ``CONVQA_PROFILE``, named once in
    :data:`_PROFILE_ENV`.
    ...
    ``os.environ.get("CONVQA_PROFILE", "local")``: that fallback reads an
...
_PKG = "conversation_qa_scorecard"
...
DEFAULT_BINDINGS: dict[str, dict[str, str]] = {
    ...
    # Rule R1: screens the narration and signal-classifier calls' input (before) and output
    # (after). local is a deterministic heuristic stand-in; there is no offline emulator for
    # Model Armor.
    "guardrail": {
    ...
    "scorecard_store": {
        "local": f"{_PKG}.adapters.local.scorecard_store:LocalScorecardStore",
        "gcp": f"{_PKG}.adapters.gcp.scorecard_store:FirestoreScorecardStore",
        "onprem": f"{_PKG}.adapters.onprem.scorecard_store:OnPremScorecardStore",
    },
    "signal_classifier": {
        "local": f"{_PKG}.adapters.local.signals:LocalSignalClassifier",
        "gcp": f"{_PKG}.adapters.gcp.signals:GeminiSignalClassifier",
        "onprem": f"{_PKG}.adapters.onprem.signals:OnPremSignalClassifier",
    },
    "transcription": {
        "local": f"{_PKG}.adapters.local.transcription:FixtureTranscriptSource",
        "gcp": f"{_PKG}.adapters.gcp.transcription:BatchTranscriptSource",
        "onprem": f"{_PKG}.adapters.onprem.transcription:OnPremTranscriptSource",
    },
    "warehouse": {
...
def _read_settings_file(path: Path | None = None) -> dict[str, Any]:
    ...
    An EXPLICIT path (argument or ``CONVQA_SETTINGS``) that does not
...
REVIEW_ROUTING_ENV = "CONVQA_REVIEW_ROUTING"

#: The switch for the guardrail (rule R1), read the same three-state way. While this is on, the
#: narration and signal-classifier calls are screened input-before / output-after
#: (``domain/scorecard_service.py``); off binds
...
GUARDRAIL_ENV = "CONVQA_GUARDRAIL"
...
class ModelArmorSettings:
    ...
    ``template_id`` has a REAL default (``conversation-qa-scorecard-guardrail``), matching the
...
class Settings:
    ...
    #: accepted paraphrases, ordering, timing windows and severities are the bank's policy on a
    #: per-market review schedule (practice B4), and changing one must never need a release.
    score_pack_path: str = ""
    #: An optional SIBLING disclosure pack (the `kind: disclosure` artifact the contact-centre
    #: copilot reminds from). Reading it rather than re-authoring its wordings is what makes "a
    #: market's requirement is reviewed once" true across the two systems. It SUPPLEMENTS the
    #: pack above and never replaces it: it carries no ordering and no named instrument.
    sibling_disclosure_pack_path: str = ""
    #: Offline scorecard store (SQLite). ``:memory:`` is the ephemeral default the gate uses.
    scorecard_path: str = ":memory:"
    #: Directory of offline transcript fixtures the ``local`` transcript source reads. Empty
    #: selects the obviously synthetic set shipped inside the package.
    transcript_fixture_path: str = ""
    #: Where the offline warehouse export appends JSON Lines. Empty means the export is held in
    #: memory only, which is right for the gate and wrong for anything durable.
    warehouse_path: str = ""
    ...
    def load(cls, path: Path | None = None) -> Settings:
        ...
        settings = cls(
            ...
            score_pack_path=str(data.get("score_pack_path") or ""),
            sibling_disclosure_pack_path=str(data.get("sibling_disclosure_pack_path") or ""),
            scorecard_path=str(data.get("scorecard_path") or ":memory:"),
            transcript_fixture_path=str(data.get("transcript_fixture_path") or ""),
...
def _refuse_unconfigured_controls(settings: Settings) -> None:
    ...
    screen call. Both are configuration errors, so both refuse here, with the two ways out for
    each.
    """
...
class Container:
    ...
    @cached_property
    def transcription(self) -> TranscriptSourcePort:
        adapter = self._bind("transcription")
        assert isinstance(adapter, TranscriptSourcePort)
        return adapter

    @cached_property
    def scorecard_store(self) -> ScorecardStorePort:
        adapter = self._bind("scorecard_store")
        assert isinstance(adapter, ScorecardStorePort)
        return adapter
    ...
    @cached_property
    def signal_classifier(self) -> SignalClassifierPort:
        adapter = self._bind("signal_classifier")
        assert isinstance(adapter, SignalClassifierPort)
        return adapter
