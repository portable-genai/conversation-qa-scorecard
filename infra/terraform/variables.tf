...
#         putting conversation evidence out of jurisdiction. The default is asia-southeast1.
...
variable "name_prefix" {
  description = <<-EOT
    Prefix for every named resource this stack creates (key ring, buckets, dataset, service
...
variable "allowed_regions" {
  description = <<-EOT
    ...
    Firestore, BigQuery, Cloud KMS, Cloud Logging, Cloud Run) and the residency obligation for
    recorded conversations are both satisfied in that region. This list is the same allowlist
    the application validates its configured region against at settings load.
  EOT
  type        = list(string)
...
variable "region" {
  description = <<-EOT
    ...
    region fails at `terraform plan` rather than moving call recordings, transcripts and
    scorecards out of jurisdiction (P-03).
  EOT
  type        = string
  default     = "asia-southeast1"
...
variable "additional_resource_locations" {
  description = <<-EOT
    ...
    own location group. The residency-critical data plane (Cloud Run, KMS, Firestore,
    BigQuery, Cloud Logging, Cloud Storage, Speech-to-Text) is entirely regional and needs
    nothing here. The serving edge, however, is built from location-less global objects (the
    IP address, URL map, target proxy, certificate and forwarding rule); an organisation whose
    policy evaluation covers those must list the value its policy expects for them here rather
    than widening the regional allowlist. Empty is the strict default.
  EOT
  type        = list(string)
  default     = []
}
...
variable "alert_notification_channels" {
  description = <<-EOT
    ...
    escalations, service-account key creation, VPC-SC denials, CMEK changes, edge denials). An
    empty list still creates the metrics and policies, with nowhere to notify.
  EOT
  type        = list(string)
  default     = []
...
variable "create_firestore_database" {
  description = <<-EOT
    Create the Firestore (default) database that backs the scorecard store. The managed store
...
variable "human_review_url" {
  description = <<-EOT
    ...
    instead of at startup. HTTPS is required, because the payload carries a redacted scorecard.
  EOT
  type        = string
  default     = ""
...
variable "iap_audience" {
  description = <<-EOT
    ...
    (CONVQA_IAP_AUDIENCE): "/projects/<PROJECT_NUMBER>/global/backendServices/<BACKEND_ID>".
  ...
  validation {
    condition = alltrue([
      ...
      !contains([
        ...
        "CONVQA_SETTINGS",
        "CONVQA_IAP_AUDIENCE",
        "CONVQA_WAREHOUSE_TABLE",
        "CONVQA_QUALITY_URL",
        "CONVQA_REVIEW_ROUTING",
...
variable "edge_iap_enabled" {
  description = <<-EOT
    ...
    assertion's signature against IAP's own key set, its audience against CONVQA_IAP_AUDIENCE,
...
variable "iap_members" {
  description = <<-EOT
    ...
    "group:qa-reviewers@example.com". Empty grants nobody, which is the correct default: IAP
