...
# (src/conversation_qa_scorecard/adapters/gcp/audit.py) writes to the logger
# "conversation_qa_scorecard-audit" as a code constant, and the WORM sink filter must name the
# same log or it routes nothing. Deriving it from the prefix here would produce a filter that
# quietly matches an empty stream. Change one and change the other in the same commit.
...
locals {
  ...
  kms_ring_name = "${var.name_prefix}-scorecard-ring"
  kms_key_name  = "${var.name_prefix}-scorecard-cmek"

  # WORM audit trail (logging_worm.tf, monitoring.tf).
  worm_bucket_id  = "${var.name_prefix}-scorecard-worm"
  audit_sink_name = "${var.name_prefix}-audit-to-worm"

  # Fixed by the application, not by this stack. See the header note above.
  audit_log_name = "conversation_qa_scorecard-audit"
  ...
  recordings_bucket_name = "${var.project_id}-${var.name_prefix}-recordings"

  # Warehouse (warehouse.tf). Dataset ids allow letters, digits and underscores only.
  warehouse_dataset_id = "${local.prefix_u}_scorecards"
  warehouse_table_id   = "scorecards"
