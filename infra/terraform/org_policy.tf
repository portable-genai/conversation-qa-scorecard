...
resource "google_org_policy_policy" "resource_locations" {
  ...
  spec {
    rules {
      values {
        # For the default region this is "in:asia-southeast1-locations": the region plus its
        # zones and sub-locations, and nothing else.
...
# Require uniform bucket-level access on every bucket (no legacy object ACLs). Call recordings
# are the most sensitive object this service reads; per-object ACLs are how one becomes public.
