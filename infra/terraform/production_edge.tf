...
# signature against IAP's own key set, its audience against CONVQA_IAP_AUDIENCE, its expiry and
# its issuer before reading a single claim. With IAP off, the exposure guard has still stood
# down under the gcp profile, so every end-user route simply answers 401 and nobody can be
# authenticated at all.
#
# What is NOT here: a UI service. This repo builds one image (the API), and the ui/
...
resource "google_cloud_run_v2_service" "api" {
  ...
  template {
    ...
    containers {
      ...
      # names nothing: CONVQA_QUALITY_URL emptied means no promotion authority and refuses,
      # and an emptied audience refuses every caller. Writing "" here would therefore be a
      # different, louder posture than leaving the variable off the service.
      dynamic "env" {
        for_each = var.iap_audience == "" ? [] : [var.iap_audience]
        content {
          name  = "CONVQA_IAP_AUDIENCE"
          value = env.value
        }
      }
...
resource "google_compute_security_policy" "api_per_source" {
  ...
  rule {
    ...
    description = "Per-source abuse boundary. Scoring a contact costs a transcript fetch and a model call, so the ceiling is at the edge rather than in the request path."
  }
