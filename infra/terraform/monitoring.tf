...
resource "google_monitoring_alert_policy" "security" {
  ...
  documentation {
    content   = "Security signal '${each.key}' fired for the E3 conversation QA scorecard. Investigate the matching entries in Cloud Logging and in the WORM audit bucket (${local.worm_bucket_id})."
