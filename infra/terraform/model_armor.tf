...
#   R1 (guardrail screening): adapters/gcp/guardrail.py screens the narration and
#         signal-classifier calls' input and output through this template for prompt
#         injection, jailbreak, sensitive-data leakage and malicious URLs, in both directions.
#   P-05 / P-03 (residency): the template is created in var.region and called on the regional
#         Model Armor host (config/settings.yaml model_armor.host), never the global endpoint.
#
# template_id matches the default config/settings.yaml model_armor.template_id
# ("conversation-qa-scorecard-guardrail"), so a zero-edit deploy names a template this file
...
resource "google_model_armor_template" "guardrail" {
  ...
  location    = var.region
  template_id = "conversation-qa-scorecard-guardrail" # matches settings.yaml model_armor.template_id
