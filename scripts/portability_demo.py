...
* the trail exports to an open format, carries its chain anchor with it, and reloads into a
  foreign store with its chain intact while a truncated export is refused;
...
from conversation_qa_scorecard import (
    config,
)
from conversation_qa_scorecard.domain import (
    kernel,
)
from conversation_qa_scorecard.ports import (
    PORT_PROTOCOLS,
)
...
def _event() -> kernel.AuditEvent:
    return kernel.AuditEvent(
        ...
        redacted_summary="CT-SG-0002: non_compliant on SG/sg-retail-banking-v1",
    )
