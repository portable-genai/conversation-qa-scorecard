...
def _check_routine(state: dict[str, Any]) -> list[str]:
    facts = _facts(state)
    problems: list[str] = []
    if facts.get("disposition") != "compliant":
        problems.append(
            "the clean contact did not score compliant: " + str(facts.get("disposition"))
        )
    ...
    if state["totals"]["escalated"] != 0:
        problems.append("escalation count moved on a compliant contact")
    if float(facts.get("disclosure_score", 0.0)) < 1.0:
        problems.append("a compliant contact reported incomplete disclosure coverage")
    if int(facts.get("cited_findings", 0)) < 1:
        problems.append("no finding cited the words that satisfied it")
    return problems
...
def _check_escalation(state: dict[str, Any]) -> list[str]:
    facts = _facts(state)
    problems: list[str] = []
    if facts.get("disposition") != "non_compliant":
        problems.append("a missing mandated disclosure did not score non-compliant")
    if facts.get("severity") != "critical":
        problems.append("a missing capital-at-risk warning was not scored critical")
    if not facts.get("requires_human_review"):
        problems.append("a non-compliant scorecard did not escalate")
...
def _check_redaction(state: dict[str, Any]) -> list[str]:
    ...
    if facts.get("planted_identifier_leaked"):
        problems.append("the planted identifier survived into the scored text or the record")
    if int(facts.get("redaction_spans", 0)) < 1:
        problems.append("no identifier was masked, so the redaction claim proves nothing")
    if not facts.get("review_ref"):
        problems.append("rule R8: the failing scorecard with personal data was not routed")
    return problems
...
def _check_audit(state: dict[str, Any]) -> list[str]:
    ...
    if int(facts.get("entries", 0)) < 3:
        problems.append("fewer audit records than contacts scored")
    return problems
